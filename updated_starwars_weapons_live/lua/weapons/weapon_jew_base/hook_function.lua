...
function SWEP:DrawWorldModel()
	...
	if IsValid(self:GetHook()) and IsValid(self.Owner) then

		render.SetMaterial( HookCable )

		render.DrawBeam( self:GetHook():GetPos(), att and att.Pos or self.Owner:GetShootPos(), 1, 0, 2, Color(255,255,255,255) )
...
function SWEP:PreDrawViewModel()
	...
	render.DrawBeam( hk:GetPos(), pos, 1, 0, 2, Color(255,255,255,255) )
...
function SWEP:PlayerMove( ply, mv, cmd )
	...
	if ply:KeyPressed( IN_USE ) and ply==self.Owner then
		...
		if hk:GetPos()[3] > ply:GetShootPos()[3] then
			...
			mv:SetVelocity( mv:GetVelocity() + Vector(0,0,400) )
